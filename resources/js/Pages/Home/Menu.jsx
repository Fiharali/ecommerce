...
function Menu({ auth }) {
    ...
    return (
        <div>
            ...
            <div className={` offcanvas offcanvas-start  ${darkMode ? 'canvassDark' : 'canvass'}`} data-bs-scroll="true" data-bs-backdrop="false" tabIndex={-1} id="offcanvasScrolling" aria-labelledby="offcanvasScrollingLabel">
                <div className="offcanvas-header">
                    <h5 className="offcanvas-title" id="offcanvasScrollingLabel"><img src="/images/logo.png" width={69} alt=""  /></h5>
                ...
                <div className="offcanvas-body">
                    ...
                    </div>
                    {
                        auth.user ? (
                            <>
                                <div className={` d-block  p-3   my-4 fs-4 rounded-5 fst-italic border-bottom border-primary   ${darkMode ? 'itemmsDark' : 'itemms'}`} onClick={Profile}>
                                    <i className="fa-solid fa-user fa-xl"></i>
                                    <Link className='ps-4 link' href={route('profile.edit')} >Profile</Link>
                                </div>
                                <div className={` d-block  p-3   my-4 fs-4 rounded-5 fst-italic border-bottom border-primary   ${darkMode ? 'itemmsDark' : 'itemms'}`}  onClick={logOut}>
                                    <i className="fa-solid fa-user fa-xl"></i>
                                    <Link className='ps-4 link' href={route('logout')} method="post"  as="button">Log Out</Link>
                                </div>
                            </>
                        ) : (
                            <>
