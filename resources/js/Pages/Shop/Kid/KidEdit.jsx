...
function KidEdit({ auth, kid }) {

    const { get, data, setData, post, errors, progress } = useForm({

        title: kid.title,
        discreption: kid.discreption,
        oldPrice: kid.oldPrice,
        price: kid.price,
        img1: '',
        img2: '',
        img3: '',
        ...
        <>
            <div className='contt'>
                ...
                <div className=' col-7 mx-auto d-flex justify-content-evenly  '>
                    <img src={`/products/${kid.img1}`} alt="" width={100} />
                    <img src={`/products/${kid.img2}`} alt="" width={100} />
                    <img src={`/products/${kid.img3}`} alt="" width={100} />
                </div>
                <div className="col-lg-4 col-md-8 col-ms-4  mx-auto my-5">
                    <form enctype="multipart/form-data" onSubmit={EditKid} className='py-5  subscribe animate__animated animate__zoomIn text ' >
                        <p className='m-2'>form Edit Women Clothes</p>
                        <div className="mb-1 mt-5">
                            <input type="text" name='title' placeholder={errors.title ? 'This Input Is For "title Kid" Required ' : "title Kid"} className={`subscribe-input   ${errors.title && 'red'}`}
                        ...
                        <div className="mb-3">
                            <input type="number" name='oldPrice' placeholder={errors.oldPrice ? 'This Input Is For "oldPrice Kid" Required  ' : "oldPrice Kid"} className={`subscribe-input   ${errors.oldPrice && 'red'}`}
                                value={data.oldPrice} onChange={(e) => setData("oldPrice", e.target.value)} />
                        </div>
                        <div className="mb-3">
                            <input type="number" name='price' placeholder={errors.price ? 'This Input Is For "price Kid" Required  ' : "price Kid"} className={`subscribe-input   ${errors.price && 'red'}`}
