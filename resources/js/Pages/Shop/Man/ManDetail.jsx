...
function ManDetail({ auth, man, card, total }) {

    const datas = [
        {
            image: `/products/${man.img1}`,

        },
        {
            image: `/products/${man.img2}`,

        },
        {
            image: `/products/${man.img3}`,
    ...
    const addManToCart = (e) => {
        e.preventDefault()
        post(route('add.man.to.cart', man.id))
    ...
    const DeleteMan = () => {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                destroy(route('delete.man', man.id))
            }
        })
            ...
            <div className="container">
                <div className="row">
                    ...
                    <div className='col-lg-7 col-md-12 animate__animated animate__slideInRight'>
                        <h1 className='text-center fs-2 p-lg-5 p-3'>{man.title}</h1>
                        <p className='mx-5 text-justify p-3 text-center'>{man.discreption}</p>
                        <div className='text-center '>
                            <span className='text-opacity-25'><del>{man.oldPrice}</del></span>
                            <br />
                            <span className='fs-4 text-success p-3'> {man.price}</span>
                        </div>
                        <div className='text-center pt-5'>
                            {
                                auth.user && auth.user.isAdmin ? (
                                    <>
                                        <button className=' button1 p-2 px-5 m-2' onClick={() => { get(route('man.edit', man.id)) }}> Update
                                            <i class="fa-solid fa-pen-to-square fa-xl"></i>
                                        </button>
                                        <button className=' button2 p-2 px-5 m-2' onClick={DeleteMan} > delete
                                    ...
                                    <>
                                        <form >
                                            ...
                                            <div className="row d-flex justify-content-center mx-5" onSubmit={addManToCart}>
                                            ...
                                            <button className='button2 p-2 m-4 px-5' type='submit' onClick={addManToCart}>Add To Cart Shop
